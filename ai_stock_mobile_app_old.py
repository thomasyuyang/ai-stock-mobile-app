...
if df.empty:
    ...
    metric1.metric("Within Buy", len(df[df["Category"] == "Within Buy"]))
    metric2.metric("Within Sell", len(df[df["Category"] == "Within Sell"]))

    metric3, metric4 = st.columns(2)
    metric3.metric("Near Buy", len(df[df["Category"] == "Near Buy"]))
    metric4.metric("Near Sell", len(df[df["Category"] == "Near Sell"]))
    ...
    with tabs[2]:
        sell_df = df[df["Category"].isin(["Within Sell", "Near Sell"])].sort_values("Score", ascending=False)
        if sell_df.empty:
